	...
	class ZURCF7_Admin_Action {
		...
		*/
		...
		function action__manage_zurcf7_data_posts_custom_column( $column, $post_id ) {
		    ...
		    $user_login = get_post_meta( $post_id , ZURCF7_META_PREFIX.'user_login', true );
		    $user = get_user_by("login", $user_login);

            if(isset($user->ID)){
                $country = get_user_meta( $user->ID , 'register_country', true ) ;
                $degree = get_user_meta( $user->ID , 'register_degreetype', true ) ;
                switch ( $column ) {
                    ...
                    case ZURCF7_META_PREFIX.'country' :
                        echo ( is_array( $country ))  ? $country[0] : '' ;
                        break;

                    case ZURCF7_META_PREFIX.'degree' :
                        echo ( is_array( $degree) ) ? $degree[0]: '' ;
                        break;
